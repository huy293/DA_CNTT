...
router.post("/", authMiddleware.protect("admin", "moderator"), movieController.createMovie); // Thêm phim
router.put("/:id", authMiddleware.protect("admin", "moderator"), movieController.updateMovie); // Sửa phim
router.delete("/:id", authMiddleware.protect("admin"), movieController.deleteMovie); // Xóa phim (chỉ admin)
