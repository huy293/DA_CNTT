...
const seasonColumns = (onEdit, onDelete) => [
  ...
  {
    name: "Ngày phát hành",
    selector: (row) => formatDate(row.release_date),
    width: "150px",
  },
  {
    name: "Trạng thái",
...
Thông tin mùa phim:
- ID: ${row.id}
- Số mùa: ${row.season_number}
- Tiêu đề: ${row.title}
- Mô tả: ${row.overview}
- Ngày phát hành: ${formatDate(row.release_date)}
- Trạng thái: ${row.status}
- Thời lượng: ${row.runtime || 'Chưa cập nhật'} phút
- URL Poster: ${row.poster_url || 'Chưa cập nhật'}
- URL Backdrop: ${row.backdrop_url || 'Chưa cập nhật'}
- URL Trailer: ${row.trailer_url || 'Chưa cập nhật'}
`;
            alert(info);
          }}
